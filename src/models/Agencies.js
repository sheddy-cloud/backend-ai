const { getRows, getRow, execute } = require('../database/config');

// Agency model using raw PostgreSQL queries
class Agency {
  constructor(data) {
    this.id = data.id;
    this.name = data.name;
    this.description = data.description;
    this.location = data.location;
    this.rating = data.rating;
    this.isActive = data.isActive;
    this.createdAt = data.createdAt;
    this.updatedAt = data.updatedAt;
  }

  static async find(options = {}) {
    try {
      let query = 'SELECT * FROM agencies WHERE deleted_at IS NULL';
      const params = [];
      let paramCount = 0;

      if (options.isActive !== undefined) {
        paramCount++;
        query += ` AND is_active = $${paramCount}`;
        params.push(options.isActive);
      }

      query += ' ORDER BY rating DESC';

      if (options.limit) {
        paramCount++;
        query += ` LIMIT $${paramCount}`;
        params.push(options.limit);
      }

      if (options.skip) {
        paramCount++;
        query += ` OFFSET $${paramCount}`;
        params.push(options.skip);
      }

      const agencies = await getRows(query, params);
      return agencies.map(agency => new Agency(agency));
    } catch (error) {
      console.error('Error finding agencies:', error);
      throw error;
    }
  }

  static async findById(id) {
    try {
      const agency = await getRow(
        'SELECT * FROM agencies WHERE id = $1 AND deleted_at IS NULL',
        [id]
      );
      return agency ? new Agency(agency) : null;
    } catch (error) {
      console.error('Error finding agency by ID:', error);
      throw error;
    }
  }
}

module.exports = Agency;
