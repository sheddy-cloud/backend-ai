...
echo "🔧 Fixing Pip Installation & Missing Dependencies"
echo "================================================="

# Navigate to application directory
cd /opt/napasa-ai-backend

echo "1. Installing build tools and fixing pip..."
sudo apt update
sudo apt install -y python3.12-dev python3.12-venv python3-pip build-essential libssl-dev libffi-dev python3-setuptools

echo "2. Stopping ML service..."
pm2 stop napasa-ml-service
pm2 delete napasa-ml-service

echo "3. Recreating Python virtual environment with proper tools..."
cd ml_service
...
cat > ecosystem.ml-working.config.js << 'EOF'
module.exports = {
  apps: [
    {
...
pm2 start ecosystem.ml-working.config.js
...
echo ""
echo "🎉 Pip and dependencies fix completed!"
echo ""
echo "📋 PM2 Status:"
pm2 status
echo ""
